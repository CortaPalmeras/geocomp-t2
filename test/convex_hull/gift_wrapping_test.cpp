...
TEST(gift_wrapping_test, cubo_con_puntos_en_aristas) {
    ...
    vector<Punto<float>> puntos_esperados({
        esquina_ii,
        esquina_id,
        esquina_sd,
        esquina_si
...
TEST(gift_wrapping_test, robustez) {
    vector<Punto<float>> puntos;
    std::uniform_real_distribution<float> dist_radio(0, 5);
    std::uniform_real_distribution<float> dist_angulo(0, 3);
